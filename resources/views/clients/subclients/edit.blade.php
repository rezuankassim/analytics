...
<div class="flex flex-col">
    <div class="px-6 py-6">
        <div class="flex items-center text-sm text-gray-500">
            ...
            <div class="flex items-center">
                <svg class="w-6 h-6 stroke-current" xmlns="http://www.w3.org/2000/svg" fill="none"><path d="M9 5l7 7-7 7" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
                <span class="text-green-500">{{ $subclient->name }}</span>
            </div>
        </div>
    </div>

    <form action="{{ route('subclients.update', ['client' => $client->id, 'subclient' => $subclient]) }}" class="w-full" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
                        ...
                        <div>
                            <div class="flex">
                                <div class="w-1/3 mr-2">
                                    ...
                                    <input id="name" name="name" class="form-input bg-gray-200 block border border-transparent w-full pl-7 pr-12 text-sm leading-5 focus:bg-white focus:outline-none focus:shadow-none focus:border-green-500 @error('name') border-red-600 text-red-800 @enderror" placeholder="piedpiper" value="{{ old('name') ?? $subclient->name }}"/>
                            ...
                            <div class="mt-2">
                                <div class="w-2/3">
                                    ...
                                    <input id="appId" name="app_id" class="form-input bg-gray-200 block border border-transparent w-full pl-7 pr-12 text-sm leading-5 focus:bg-white focus:outline-none focus:shadow-none focus:border-green-500 @error('app_id') border-red-600 text-red-800 @enderror" placeholder="com.piedpiper" value="{{ old('app_id') ?? $subclient->app_id }}" autocomplete="off"/>
                            ...
                            <div class="mt-2">
                                <div class="w-full">
                                    ...
                                    <textarea id="description" name="description" class="form-textarea resize-none h-32 bg-gray-200 block border border-transparent w-full pl-7 pr-12 text-sm leading-5 focus:bg-white focus:outline-none focus:shadow-none focus:border-green-500" placeholder="Pied Piper is a multi-platform technology based on a proprietary universal compression algorithm that has consistently fielded high Weisman Score that are not merely competitive, but approach the theoretical limit of lossless compression.">{{ old('description') ?? $subclient->description }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
